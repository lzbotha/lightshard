...
public class CharacterLightController : MonoBehaviour {
	...
	void Start () {
		...
		this.characterState.setLightRegenRate(normalLightRegenRate);
		this.characterState.setLightRadius(normalLightRadius);
		this.characterState.setMinLightRadius(minLightRadius);
	}

	public void disableFlash() {
		this.characterState.setLightRegenRate(this.normalLightRegenRate);
		this.characterState.setFlashDeactivationRadius(0);
	}
	...
	void Update () {
		...
		if(this.characterState.isLightRegenNegative() && this.characterState.getFlashDeactivationRadius() > 0){
			// Decrease light radius
			this.characterState.changeLightRadiusBy(this.characterState.getLightRegenRate() * Time.deltaTime);
			// If the flash duration is over, or the character is at minimum light 
			// radius, restore the characters light regen to normal
			if(this.characterState.getLightRadius() <= this.characterState.getFlashDeactivationRadius()){
				this.disableFlash();
			}
		}
		...
		// 			can't be in an else
			...
			if(this.characterState.getLightRadius() + this.characterState.getLightRegenRate() * Time.deltaTime <= this.normalLightRadius)
				this.characterState.changeLightRadiusBy(this.characterState.getLightRegenRate() * Time.deltaTime);
		}
		this.gameObject.light.range = characterState.getLightRadius();
	}
}
